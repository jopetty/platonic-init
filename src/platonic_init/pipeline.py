...
def main() -> None:
    ...
    subspace_path = artifacts / "weight_subspace.pt"
    torch.save(subspace, subspace_path)
    with (artifacts / "weight_subspace_summary.json").open("w", encoding="utf-8") as f:
        json.dump(build_summary(subspace), f, indent=2)

    analytic_subspace, fit_report = fit_analytic_subspace(subspace, cfg.analytic_fit)
    analytic_path = artifacts / "analytic_subspace.pt"
    torch.save(analytic_subspace, analytic_path)
    with (artifacts / "analytic_fit_report.json").open("w", encoding="utf-8") as f:
    ...
    with (artifacts / "init_eval.json").open("w", encoding="utf-8") as f:
