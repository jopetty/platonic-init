...
def gpt2_permutation_spec(
    ...
    *,
    ...
    for layer_id in layer_ids:
        ...
        if (
            attn_qkv_w not in state
            or attn_qkv_b not in state
            or attn_proj_w not in state
        ):
            continue
        ...
        axes_to_perm[attn_qkv_w] = (
            None,
            AxisPermutation(attn_perm, group_size=head_dim, num_blocks=3),
        )
        axes_to_perm[attn_qkv_b] = (
            AxisPermutation(attn_perm, group_size=head_dim, num_blocks=3),
        )
        axes_to_perm[attn_proj_w] = (
            AxisPermutation(attn_perm, group_size=head_dim, num_blocks=1),
            None,
        )
...
def _expanded_axis_indices(
    ref: AxisPermutation, perm_idx: torch.Tensor
) -> torch.Tensor:
    blocks = []
...
def _flatten_rows_for_matching(
    w: torch.Tensor, n: int, ref: AxisPermutation
) -> torch.Tensor:
    g = int(ref.group_size)
    ...
    if int(w.shape[0]) != expected:
        raise ValueError(
            "Unexpected axis size for grouped permutation: "
            f"got {w.shape[0]}, expected {expected}"
        )
...
def get_permuted_param(
    ...
    for axis, perm_ref in enumerate(ps.axes_to_perm.get(key, ())):
        if axis == except_axis or perm_ref is None:
            continue
        idx = _expanded_axis_indices(perm_ref, perm[perm_ref.perm_name]).to(
            device=w.device
        )
...
def apply_permutation(
    ...
    return {
        key: get_permuted_param(ps, perm, key, params)
        if key in ps.axes_to_perm
        else value
    ...
    for perm_name, axes in ps.perm_to_axes.items():
        ...
        for wk, axis in axes:
            ...
            if denom <= 0 or axis_size % denom != 0:
                raise ValueError(
                    f"Invalid grouped permutation axis for {wk}: "
                    f"size {axis_size}, denom {denom}"
                )
        ...
        if any(n != n0 for n in n_candidates):
            raise ValueError(
                f"Inconsistent permutation size for {perm_name}: {n_candidates}"
            )
        perm_sizes[perm_name] = n0

    perm = {
        perm_name: torch.arange(n, dtype=torch.long)
        for perm_name, n in perm_sizes.items()
    }
    if not perm:
        return perm
    ...
    for _ in range(max_iter):
        ...
        for perm_index in rng.permutation(len(perm_names)):
            ...
            for wk, axis in ps.perm_to_axes[perm_name]:
                ...
                w_b = (
                    get_permuted_param(ps, perm, wk, params_b, except_axis=axis)
                    .detach()
                    .to(
                        dtype=torch.float64,
                        device="cpu",
